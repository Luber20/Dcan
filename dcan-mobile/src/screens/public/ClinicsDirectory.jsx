...
const ECUADOR_DATA = {
  ...
  Carchi: ["Tultcán", "Bolívar", "Espejo", "Mira", "Montúfar"],
  ...
  Manabí: ["Portoviejo", "Manta", "Chone", "Montecristi", "Jipijapa", "Pedernales", "El Carmen","Jaramijó"],
...
export default function ClinicsDirectory() {
  ...
  const filteredClinics = useMemo(() => {
    ...
    return clinics.filter((c) => {
      ...
      const matchesSearch =
        q === "" || `${c.name} ${c.address} ${clinicProv} ${clinicCant}`.toLowerCase().includes(q);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.headerContainer}>
        <View style={styles.headerTop}>
          ...
          {/* ✅ BOTÓN NUEVO: Iniciar sesión (sin necesidad de agendar) */}
          {!user && (
            <Button
              mode="contained"
              buttonColor="#FFFFFF"
            ...
            renderItem={({ item }) => (
              <List.Item
                ...
                left={(props) => (
                  <List.Icon
                    {...props}
                    icon={selectedProvince === item ? "check-circle" : "map-marker-outline"}
                    color={selectedProvince === item ? "#2E8B57" : "#888"}
                  />
                )}
              />
            )}
              ...
              renderItem={({ item }) => (
                <List.Item
                  ...
                  left={(props) => (
                    <List.Icon
                      {...props}
                      icon={selectedCanton === item ? "check-circle" : "city-variant-outline"}
                      color={selectedCanton === item ? "#2E8B57" : "#888"}
                    />
                  )}
                />
              )}
...
const styles = StyleSheet.create({
  ...
  logo: { width: 80, height: 80, marginRight: 15, borderRadius: 10 },
  headerTitles: { flex: 1 },
