...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      // Guardamos el usuario con su clinic_id
      setUser({ ...user, clinic_id });

      // LA MODIFICACIÓN: Extraemos el nombre del rol y lo devolvemos
    // Como Laravel envía un array de roles, tomamos el primero.
    const roleName = user.roles && user.roles.length > 0 ? user.roles[0].name : 'client';

    return { success: true, role: roleName }; // <--- Ahora enviamos el rol de vuelta
