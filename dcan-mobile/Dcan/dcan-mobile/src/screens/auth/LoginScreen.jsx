...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    // Ahora recibimos el resultado que incluye el 'role' (gracias al cambio en AuthContext)
    const result = await login(email.trim().toLowerCase(), password);
    setLoading(true); // Se mantiene cargando mientras decide la ruta

    if (result.success) {
      // REDIRECCIÓN POR ROL
      // 'veterinarian' es el nombre que pusiste en tu Seeder de Laravel
      if (result.role === 'veterinarian') {
        navigation.replace("VeterinarianTabs"); 
      } else if (result.role === 'clinic_admin') {
        navigation.replace("AdminHome");
      } else {
        navigation.replace("MainTabs"); // Home para clientes
      }
    } else {
      setLoading(false);
